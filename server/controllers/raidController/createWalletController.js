...
const initializeUserPool = async (req, res) => {
  try {
    ...
    while (blockheight < lastValidBlockHeight) {
      const response = await solConnection.sendTransaction(newTx, [oldWallet]);
    ...
    // res.send({ msg: "pool created", YourWallet: oldWallet.publicKey.toString(), tx: response, type: "success" });
  } catch (e) {
    console.log(e.message, " err-in createUserController");
    res.status(500).send({ msg: e.message, type: "failed" });
  }
